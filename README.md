...
22. [Medical Intelligence Atlas Coverage Dashboard](docs/medical_intelligence_atlas_coverage_dashboard.md): machine readable coverage dashboard that maps fourteen atlas nodes to local runtime artifacts, validators, coverage focus, and open gap state.
...
Before using any route state in public text, run `make public_github_route_preflight`. It checks the watchlist, live GitHub route state, README surface links, and public release sanitation. Before using the internal medicine safety lane in public work, run `make internal_medicine_ai_safety_strategy`. Before using Clinical Intelligence Stack language in public work, run `make clinical_intelligence_stack`. Before using the Multilingual Medical Intelligence paired state check, run `make multilingual_medical_intelligence_source_check`. Before using the Multilingual Medical Intelligence public wording bank, run `make multilingual_medical_intelligence_public_wording_bank`; it checks passing rows, expected fail drift controls, drift signal counts, failure reason counts, reviewer triage, rewrite candidate scoring, blocked candidate counts, cross language ambiguity controls, cross language negation audience controls, cross language scope anchor controls, and translation clearance absence without score certification. Before using rewrite candidates for public wording review, run `make multilingual_medical_intelligence_rewrite_candidate_drift_scorer`; it checks sixteen candidate rows, eight pass candidates, eight blocked candidates, candidate drift signals, source row coverage, cross language ambiguity signal counts, negation inversion and audience role signal counts, scope anchor signal counts, and external publication clearance absence. Before using cross language ambiguity controls, run `make multilingual_medical_intelligence_cross_language_ambiguity_controls`; it checks twelve Turkish English controls, six pass controls, six blocked controls, six ambiguity signal types, translation clearance absence, and external publication clearance absence. Before using cross language negation audience controls, run `make multilingual_medical_intelligence_cross_language_negation_audience_controls`; it checks twelve Turkish English controls, six pass controls, six blocked controls, six negation and audience role signal types, audience role clearance absence, and external publication clearance absence. Before using cross language scope anchor controls, run `make multilingual_medical_intelligence_cross_language_scope_anchor_controls`; it checks twelve Turkish English controls, six pass controls, six blocked controls, six scope anchor signal types, scope anchor clearance absence, and external publication clearance absence. Before using the global distribution map in public work, run `make clinical_intelligence_stack_global_target_map`. Before using the Medical Intelligence Atlas in public work, run `make medical_intelligence_atlas`; it now checks the atlas registry, coverage dashboard, release gate, local artifact paths, multilingual paired state source checks, public wording bank checks, drift negative controls, public wording drift score report, rewrite candidate score report, cross language ambiguity report, cross language negation audience report, cross language scope anchor report, and exact next action state.
