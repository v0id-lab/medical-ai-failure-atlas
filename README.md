...
Before using any route state in public text, run `make public_github_route_preflight`. It checks the watchlist, live GitHub route state, README surface links, and public release sanitation. Before using the internal medicine safety lane in public work, run `make internal_medicine_ai_safety_strategy`. Before using Clinical Intelligence Stack language in public work, run `make clinical_intelligence_stack`. Before using the Multilingual Medical Intelligence paired state check, run `make multilingual_medical_intelligence_source_check`. Before using the Multilingual Medical Intelligence public wording bank, run `make multilingual_medical_intelligence_public_wording_bank`; it checks passing rows, expected fail drift controls, drift signal counts, failure reason counts, reviewer triage, rewrite candidate scoring, and blocked candidate counts without score certification. Before using rewrite candidates for public wording review, run `make multilingual_medical_intelligence_rewrite_candidate_drift_scorer`; it checks sixteen candidate rows, eight pass candidates, eight blocked candidates, candidate drift signals, source row coverage, and external publication clearance absence. Before using the global distribution map in public work, run `make clinical_intelligence_stack_global_target_map`. Before using the Medical Intelligence Atlas in public work, run `make medical_intelligence_atlas`; it now checks the atlas registry, coverage dashboard, release gate, local artifact paths, multilingual paired state source checks, public wording bank checks, drift negative controls, public wording drift score report, rewrite candidate score report, and exact next action state.
