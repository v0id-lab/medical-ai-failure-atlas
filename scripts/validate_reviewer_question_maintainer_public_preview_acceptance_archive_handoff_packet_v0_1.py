...
MARKDOWN = ROOT / "docs" / "REVIEWER_QUESTION_MAINTAINER_PUBLIC_PREVIEW_ACCEPTANCE_ARCHIVE_HANDOFF_PACKET_V0_1.md"
...
REQUIRED_PHRASES = [
    ...
    "ready_for_public_preview_acceptance_archive_handoff_packet",
    "Boundary handoff packet row",
    "Reviewer question handoff packet row",
    "Blocked wording handoff packet row",
    "Public surface handoff packet row",
    "Validation handoff packet row",
    "Next build handoff packet row",
    "benchmark scoring",
    ...
    "Add a reviewer question maintainer public preview acceptance archive steward note without scoring",
]
FORBIDDEN_PHRASES = [
...
def main() -> int:
    ...
    if not isinstance(rows, list):
        ...
        rows = []
    expected_counts = {
        "acceptance_archive_handoff_packet_row_count": 6,
    ...
    if data.get("public_preview_acceptance_archive_handoff_packet") != "ready_for_public_preview_acceptance_archive_handoff_packet":
        errors.append("public_preview_acceptance_archive_handoff_packet must be ready_for_public_preview_acceptance_archive_handoff_packet")
    ...
    if len(rows) != 6:
        errors.append(f"Expected 6 acceptance archive handoff packet rows, found {len(rows)}")
    ...
    handoff_packet_ids = {str(row.get("handoff_packet_id")) for row in rows}
    if handoff_packet_ids != REQUIRED_HANDOFF_PACKET_IDS:
        errors.append("handoff packet id set must match required ids")
    final_index_ids = {str(row.get("source_final_index_id")) for row in rows}
    if final_index_ids != REQUIRED_FINAL_INDEX_IDS:
        errors.append("source final index id set must match required ids")
    if {str(row.get("handoff_packet_state")) for row in rows} != {"ready_for_public_preview_acceptance_archive_handoff_packet"}:
        errors.append("all handoff packet states must be ready_for_public_preview_acceptance_archive_handoff_packet")

    for row in rows:
        handoff_packet_id = str(row.get("handoff_packet_id", ""))
        for key in [
            "handoff_packet_name",
            "source_final_index_id",
            "handoff_packet_note",
            "handoff_packet_state",
            "handoff_packet_boundary",
            "handoff_packet_decision",
            "blocked_claims",
        ]:
            if key not in row:
                errors.append(f"{handoff_packet_id}: missing {key}")
    ...
    if "-" in text:
        errors.append("Generated outward facing acceptance archive handoff packet must not contain hyphen characters")

    if errors:
        print("FAIL reviewer question maintainer public preview acceptance archive handoff packet validation")
        for error in errors:
            print(f"- {error}")
        return 1

    print("PASS reviewer question maintainer public preview acceptance archive handoff packet validation")
