...
SOURCE = ROOT / "docs" / "label_audit" / "label_audit_maintainer_release_readiness_digest_v0_1.json"
MARKDOWN = ROOT / "docs" / "label_audit" / "LABEL_AUDIT_MAINTAINER_RELEASE_READINESS_DIGEST_V0_1.md"

REQUIRED_READINESS_IDS = {"LAMR001", "LAMR002", "LAMR003", "LAMR004", "LAMR005"}
REQUIRED_FILES = [
...
def main() -> int:
    ...
    if data.get("previous_public_issue_number") != 32:
        errors.append("previous_public_issue_number must be 32")
    ...
    if errors:
        print("FAIL label audit maintainer release readiness digest validation")
        for error in errors:
            print(f"- {error}")
        return 1

    print("PASS label audit maintainer release readiness digest validation")
