...
MARKDOWN = ROOT / "docs" / "REVIEWER_QUESTION_MAINTAINER_PUBLIC_PREVIEW_ACCEPTANCE_ARCHIVE_STEWARD_INDEX_V0_1.md"
...
REQUIRED_PHRASES = [
    ...
    "Acceptance archive steward index rows: 6",
    ...
    "Issue history rows represented: 12",
    "Previous public issue represented: 78",
    "current public preview route only",
    "ready_for_public_preview_acceptance_archive_steward_index",
    "Boundary steward index row",
    "Reviewer question steward index row",
    "Blocked wording steward index row",
    "Public surface steward index row",
    "Validation steward index row",
    "Next build steward index row",
    "benchmark scoring",
    ...
    "Add a reviewer question maintainer public preview acceptance archive stewardship closeout without scoring",
]
FORBIDDEN_PHRASES = [
...
def main() -> int:
    ...
    if not isinstance(rows, list):
        ...
        rows = []
    expected_counts = {
        "acceptance_archive_steward_index_row_count": 6,
        ...
        "issue_history_rows_represented": 12,
        "previous_public_issue_number": 78,
    }
    for key, value in expected_counts.items():
        if data.get(key) != value:
            errors.append(f"{key} must be {value}")
    if data.get("public_preview_acceptance_archive_steward_index") != "ready_for_public_preview_acceptance_archive_steward_index":
        errors.append("public_preview_acceptance_archive_steward_index must be ready_for_public_preview_acceptance_archive_steward_index")
    ...
    if len(rows) != 6:
        errors.append(f"Expected 6 acceptance archive steward index rows, found {len(rows)}")
    ...
    steward_index_ids = {str(row.get("steward_index_id")) for row in rows}
    if steward_index_ids != REQUIRED_STEWARD_INDEX_IDS:
        errors.append("steward index id set must match required ids")
    steward_note_ids = {str(row.get("source_steward_note_id")) for row in rows}
    if steward_note_ids != REQUIRED_STEWARD_NOTE_IDS:
        errors.append("source steward note id set must match required ids")
    if {str(row.get("steward_index_state")) for row in rows} != {"ready_for_public_preview_acceptance_archive_steward_index"}:
        errors.append("all steward index states must be ready_for_public_preview_acceptance_archive_steward_index")

    for row in rows:
        steward_index_id = str(row.get("steward_index_id", ""))
        for key in [
            "steward_index_name",
            "source_steward_note_id",
            "steward_index_note",
            "steward_index_state",
            "steward_index_boundary",
            "steward_index_decision",
            "blocked_claims",
        ]:
            if key not in row:
                errors.append(f"{steward_index_id}: missing {key}")
    ...
    if "-" in text:
        errors.append("Generated outward facing acceptance archive steward index must not contain hyphen characters")

    if errors:
        print("FAIL reviewer question maintainer public preview acceptance archive steward index validation")
        for error in errors:
            print(f"- {error}")
        return 1

    print("PASS reviewer question maintainer public preview acceptance archive steward index validation")
